...
describe('InputPanel', () => {
  ...
  it('shows the character count for the current value', () => {
    render(<InputPanel value="ab" mode="encode" onChange={vi.fn()} onClear={vi.fn()} />);
    expect(screen.getByText('2 ký tự')).toBeInTheDocument();
  });

  it('shows the encode placeholder in encode mode', () => {
    render(<InputPanel value="" mode="encode" onChange={vi.fn()} onClear={vi.fn()} />);
  ...
  it('shows the decode placeholder in decode mode', () => {
    render(<InputPanel value="" mode="decode" onChange={vi.fn()} onClear={vi.fn()} />);
    expect(screen.getByPlaceholderText('Dán chuỗi Base64 cần giải mã...')).toBeInTheDocument();
  });

  it('calls onChange as the user types', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    render(<InputPanel value="" mode="encode" onChange={onChange} onClear={vi.fn()} />);
  ...
  it('disables Clear when the input is empty, enables it otherwise', () => {
    const { rerender } = render(
      <InputPanel value="" mode="encode" onChange={vi.fn()} onClear={vi.fn()} />,
    );
    expect(screen.getByRole('button', { name: 'Clear' })).toBeDisabled();

    rerender(<InputPanel value="a" mode="encode" onChange={vi.fn()} onClear={vi.fn()} />);
  ...
  it('asks for confirmation before clearing, and only clears on confirm', async () => {
    const user = userEvent.setup();
    const onClear = vi.fn();
    render(<InputPanel value="a" mode="encode" onChange={vi.fn()} onClear={onClear} />);
