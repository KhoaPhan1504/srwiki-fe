import { Check, CircleX, Copy, Link } from 'lucide-react';
...
type Props = {
  value: string;
  mode: OperationMode;
  error: UrlEncodeDecodeError | null;
};
      ...
      <CardContent>
        {error ? (
          ...
          >
            ...
            <p className="max-w-sm text-muted-foreground">{t('output.invalidUrlEncoding')}</p>
          </div>
        ) : value ? (
          ...
          <div className="flex h-80 flex-col items-center justify-center gap-2 rounded-md border border-dashed text-center text-sm text-muted-foreground">
            <Link className="h-8 w-8 opacity-50" aria-hidden="true" />
            <p>
