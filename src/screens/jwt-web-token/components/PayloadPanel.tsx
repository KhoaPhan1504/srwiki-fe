...
export const PayloadPanel = ({ value, onChange, error }: Props) => {
  ...
  const handleCopy = async () => {
    try {
      ...
      toast.error(t('payload.copyError'), { position: 'bottom-center' });
    }
  };
  ...
  return (
    <Card className="rounded-2xl shadow-[var(--dashboard-card-shadow)]">
      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-x-4 gap-y-2 space-y-0">
        <CardTitle className="text-sm font-medium">{t('payload.title')}</CardTitle>
        <Button
          type="button"
          variant="outline"
          size="sm"
          onClick={handleCopy}
          disabled={!value}
          aria-label={copied ? t('payload.copyCopied') : t('payload.copy')}
        >
          {copied ? (
          ...
          <span className="hidden sm:inline" aria-hidden="true">
            {copied ? t('payload.copyCopied') : t('payload.copy')}
          </span>
        </Button>
      </CardHeader>
      <CardContent className="space-y-2">
        ...
        {error && (
          <p role="alert" className="text-xs text-destructive">
            {t('payload.invalidJson', { message: error })}
          </p>
        )}
      </CardContent>
    </Card>
